...
--        :addTo(self)
    ...
    self._txtIngot = cc.LabelAtlas:_create(string.formatNumberThousands(GlobalUserItem.lUserIngot,true,"/"), "Task/num_task_0.png", 16, 22, string.byte("/")) 
            ...
            :setVisible(false)
    		:addTo(self)
   ccui.Button:create("Task/text_task_task.png")
		:move(1334-675,yl.HEIGHT-135)
		:addTo(self)
        :setLocalZOrder(2)

	ccui.Button:create("public/bt_return_0.png","public/bt_return_1.png")
		:move(1334-275,yl.HEIGHT-130)
		:addTo(self)
        :setLocalZOrder(2)
...
--        self:addChild(sp)
--    end
    frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("sp_public_frame_22.png")
    if nil ~= frame then
        local sp = cc.Sprite:createWithSpriteFrame(frame)
        sp:setPosition(yl.WIDTH/2,375)
        self:addChild(sp)
...
--        :setTag(4)
--        :addTo(cell)
    local cpName = ClipText:createClipText(cc.size(700,60), item.szTaskName, "fonts/round_body.ttf", 36)
    cpName:setAnchorPoint(cc.p(0, 0.5))
    cpName:setPosition(230, 50)
    cpName:setTextColor(cc.c4b(96,59,0,255))
    cpName:setTag(5)
