...
--竣付通页面
local JunFuTongPay = class("IngotPay", cc.Layer)
local JFT_RETURN = 1
function JunFuTongPay:ctor(parent, itemname, price, paylist, token)
    ...
    --加载csb资源
    local rootLayer, csbNode = ExternalFun.loadRootCSB("Ingot/JunFuTongPay.csb", self)
...
--支付选择页面
local IngotPay = class("IngotPay", cc.Layer)
...
function IngotPay:ctor(parent)
	...
	--加载csb资源
	local rootLayer, csbNode = ExternalFun.loadRootCSB("Ingot/ShopPayLayer.csb", self)
...
function IngotPay:isPayMethodValid()
...
function IngotPay:showLayer(var)
	self:setVisible(var)
...
function IngotPay:refresh(count, name, sprice, fprice, appid)
...
function IngotPay:onButtonClickedEvent(tag, sender)
...
function IngotPay:onSelectedEvent(tag, sender)
...
function IngotPay:onTouchBegan(touch, event)
	return self:isVisible()
end

function IngotPay:onTouchEnded(touch, event)
...
function IngotPay:onJunFuTongPay(data)
    -- token参数
    local tokenparam = {}
...
function IngotPay:hide()
    self.m_spBgKuang:stopAllActions()
    self.m_spBgKuang:runAction(self.m_actHideAct)
end

--商城页面
local IngotLayer = class("IngotLayer", function(scene)
		local IngotLayer = display.newLayer(cc.c4b(0, 0, 0, 125))
    return IngotLayer
end)

IngotLayer.CBT_SCORE			= 1
IngotLayer.CBT_BEAN			= 2
IngotLayer.CBT_VIP			= 3
IngotLayer.CBT_PROPERTY		= 4
IngotLayer.CBT_ENTITY		= 5
IngotLayer.CBT_BEAN_SCORE    = 6
IngotLayer.CBT_BEAN_ROOMCARD	= 7

IngotLayer.BT_SCORE			= 30
IngotLayer.BT_VIP			= 50
IngotLayer.BT_PROPERTY		= 60
IngotLayer.BT_GOODS			= 120
IngotLayer.BT_BEAN			= 520

IngotLayer.BT_ORDERRECORD    = 1001
IngotLayer.BT_BAG            = 1002

IngotLayer.BEAN_BEAN         = 2 --豆
IngotLayer.BEAN_SCORE        = 1 --金币
IngotLayer.BEAN_ROOMCARD     = 3 --房卡
...
-- 进入场景而且过渡动画结束时候触发。
function IngotLayer:onEnterTransitionFinish()
...
-- 退出场景而且开始过渡动画时候触发。
function IngotLayer:onExitTransitionStart()
    return self
end

--scene
--stmod 进入商店后的选择类型
function IngotLayer:ctor(scene, stmod)
	stmod = stmod or IngotLayer.CBT_PROPERTY
...
--    display.newSprite("Ingot/frame_shop_0.png")
...
--	end
    ...
    topBtn:move(yl.WIDTH - 90,yl.HEIGHT-51)
        :addTo(self)
        :setTag(IngotLayer.BT_BAG)
        :addTouchEventListener(function(ref, tType)
                if tType == ccui.TouchEventType.ended then
                    local tag = ref:getTag()
                    if tag == IngotLayer.BT_ORDERRECORD then
                        this._scene:onChangeShowMode(yl.SCENE_ORDERRECORD)
                    elseif tag == IngotLayer.BT_BAG then
   ...
   display.newSprite("Ingot/sp_Ingot_bg.png")
    	:move(yl.WIDTH*0.5,yl.HEIGHT*0.5)
    	:addTo(self)
	--金币、游戏豆、元宝
--	display.newSprite("Ingot/frame_shop_2.png")
--		:move(365,80)
--		:addTo(self)
--	display.newSprite("Ingot/icon_shop_0.png")
--		:move(247,80)
--		:addTo(self)
--	display.newSprite("Ingot/frame_shop_3.png")
--		:move(679,80)
--		:addTo(self)
----	display.newSprite("Ingot/icon_shop_1.png")
----		:move(555,80)
----		:addTo(self)
--	display.newSprite("Ingot/frame_shop_4.png")
--		:move(1000,80)
--		:addTo(self)
--	display.newSprite("Ingot/icon_shop_2.png")
--		:move(877,80)
--		:addTo(self)
--    display.newSprite("Ingot/icon_shop_3.png")
--		:move(555,80)
--		:addTo(self)

	self._txtGold = cc.LabelAtlas:_create(string.formatNumberThousands(GlobalUserItem.lUserScore,true,"/"), "Ingot/num_shop_0.png", 16, 22, string.byte(".")) 
    		:move(293,80)
    		:setAnchorPoint(cc.p(0,0.5))
    		:addTo(self)
            :setVisible(false)
--    self._txtBean = cc.LabelAtlas:_create(string.formatNumberThousands(GlobalUserItem.dUserBeans,true,"/"), "Ingot/num_shop_0.png", 16, 22, string.byte(".")) 
--    		:move(603,699)
--    		:setAnchorPoint(cc.p(0,0.5))
--    		:addTo(self)
    self._txtIngot = cc.LabelAtlas:_create(string.formatNumberThousands(GlobalUserItem.lUserIngot,true,"/"), "Ingot/num_shop_0.png", 16, 22, string.byte(".")) 
    		:move(924,80)
    		:setAnchorPoint(cc.p(0,0.5))
    		:addTo(self)
            :setVisible(false)
   self._txtRoomCard = cc.LabelAtlas:_create(string.formatNumberThousands(GlobalUserItem.lUserIngot,true,"/"), "Ingot/num_shop_0.png", 16, 22, string.byte(".")) 
    		:move(603,80)
    		:setAnchorPoint(cc.p(0,0.5))
    		:addTo(self)
            :setVisible(false)

--    display.newSprite("Ingot/frame_shop_6.png")
--    	:move(178,320)
--    	:addTo(self)

    --游戏币
    ccui.CheckBox:create("Ingot/bt_shop_0_0.png","","Ingot/bt_shop_0_1.png","","")
		:move(190,530)
		:addTo(self)
		:setSelected(false)
        :setVisible(false)
        :setEnabled(false)
        :setName("check" .. 5)
		:setTag(IngotLayer.CBT_SCORE)
		:addEventListener(cbtlistener)

	--游戏豆
    ccui.CheckBox:create("Ingot/bt_shop_1_0.png","","Ingot/bt_shop_1_1.png","","")
	    :move(190,530-104)
	    :addTo(self)
	    :setSelected(false)
        :setVisible(false)
        :setEnabled(false)
        :setName("check" .. 6)
	    :setTag(IngotLayer.CBT_BEAN)
	    :addEventListener(cbtlistener)    

	--VIP
    ccui.CheckBox:create("Ingot/bt_shop_2_0.png","","Ingot/bt_shop_2_1.png","","")
		:move(190,530-104*2)
		:addTo(self)
		:setSelected(false)
        :setVisible(false)
        :setEnabled(false)
        :setName("check" .. 7)
		:setTag(IngotLayer.CBT_VIP)
		:addEventListener(cbtlistener)

	--道具
    ccui.CheckBox:create("Ingot/bt_shop_3_0.png","","Ingot/bt_shop_3_1.png","","")
		:move(190,530-104*3)
		:addTo(self)
		:setSelected(false)
        :setVisible(false)
        :setEnabled(false)
        :setName("check" .. 8)
		:setTag(IngotLayer.CBT_PROPERTY)
		:addEventListener(cbtlistener)

	--实物
    ccui.CheckBox:create("Ingot/bt_shop_4_0.png","","Ingot/bt_shop_4_1.png","","")
		:move(190,530-104*4)
		:addTo(self)
		:setSelected(false)
        :setVisible(false)
        :setEnabled(false)
        :setName("check" .. 9)
		:setTag(IngotLayer.CBT_ENTITY)
		:addEventListener(cbtlistener)
------------------------
    --游戏豆分支中的现金充值金币和现金充值房卡
    self.m_cbScoreOrRoomCard = 0
    --现金充值金币
    ccui.CheckBox:create("Ingot/bt_shop_bean_0_0.png","","Ingot/bt_shop_bean_0_1.png","","")
		...
		:setTag(IngotLayer.CBT_BEAN_SCORE)
		:addEventListener(cbtlistener)
    --现金充值房卡
    ccui.CheckBox:create("Ingot/bt_shop_bean_1_0.png","","Ingot/bt_shop_bean_1_1.png","","")
		:move(yl.WIDTH*0.5,633)
        :setAnchorPoint(cc.p(0,0.5))
		:addTo(self)
		:setSelected(false)
		...
		:setTag(IngotLayer.CBT_BEAN_ROOMCARD)
		:addEventListener(cbtlistener)
----------------------
	...
	self._scrollView = ccui.ScrollView:create()
                                      ...
                                      :setContentSize(cc.size(1100,410))
									  :setAnchorPoint(cc.p(0.5, 0.5))
									  :setPosition(cc.p(yl.WIDTH*0.49, 314))
...
function IngotLayer:updateCheckBoxList()
...
function IngotLayer:getShopPropertyType()
...
--按键监听
function IngotLayer:onButtonClickedEvent(tag,sender)
    ...
    if math.abs(endPos.x - beginPos.x) > 30 
        ...
        print("IngotLayer:onButtonClickedEvent ==> MoveTouch Filter")
        return
    end

	local name = sender:getName()

	if name == SHOP_BUY[IngotLayer.BT_SCORE] then
		--游戏币获取
		GlobalUserItem.buyItem = self._scoreList[tag-IngotLayer.BT_SCORE]
	...
	elseif name == SHOP_BUY[IngotLayer.BT_BEAN] then
		--游戏豆获取
        local item = self._beanList[tag - IngotLayer.BT_BEAN]
        if nil == item then
            return
        end
        ...
        if bThirdPay then
            if false == IngotPay:isPayMethodValid() then
                showToast(self, "支付服务未开通!", 2, cc.c4b(250,0,0,255))
                return
            end
            if nil == self.m_payLayer then
                self.m_payLayer = IngotPay:create(self)
	...
	elseif name == SHOP_BUY[IngotLayer.BT_VIP] then
		--vip购买
		GlobalUserItem.buyItem = self._vipList[tag-IngotLayer.BT_VIP]
		self:getParent():getParent():onChangeShowMode(yl.SCENE_SHOPDETAIL)
	elseif name == SHOP_BUY[IngotLayer.BT_PROPERTY] then
		--道具购买
		GlobalUserItem.buyItem = self._propertyList[tag-IngotLayer.BT_PROPERTY]
...
function IngotLayer:onSelectedEvent(tag,sender,eventType)
	...
	--游戏币
	if (tag == IngotLayer.CBT_SCORE) then
	...
	--游戏豆 -- 现金充值游戏豆
	if (tag == IngotLayer.CBT_BEAN) then
		self:onClearShowList()
        self.m_cbScoreOrRoomCard = IngotLayer.BEAN_BEAN
    ...
    --游戏豆--分支现金充值金币
	if (tag == IngotLayer.CBT_BEAN_SCORE) then
        self._select = IngotLayer.CBT_BEAN
        self.m_cbScoreOrRoomCard = IngotLayer.BEAN_SCORE
		self:onClearShowList()
		if 0 == #self._beanList then
			self:loadPropertyAndVip(tag) 
		else
    ...
    --游戏豆--分支现金充值房卡    
	if (tag == IngotLayer.CBT_BEAN_ROOMCARD) then
        self._select = IngotLayer.CBT_BEAN
        self.m_cbScoreOrRoomCard = IngotLayer.BEAN_ROOMCARD
		self:onClearShowList()
		if 0 == #self._beanList then
			self:loadPropertyAndVip(tag) 
		else
	...
	--vip
	if (tag==IngotLayer.CBT_VIP) then
	...
	--道具
	if (tag==IngotLayer.CBT_PROPERTY) then
...
--网络请求
function IngotLayer:loadPropertyAndVip(tag)	
	local this = self
    local typid = 0
    ...
    if tag == IngotLayer.CBT_ENTITY then
    	self:onUpdateGoodsList()
    --游戏豆额外处理
    elseif tag == IngotLayer.CBT_BEAN then
        if 0 ~= #self._beanList then
            self:onUpdateBeanList()
            return
        end
        ...
        if tag == IngotLayer.CBT_VIP and 0 ~= #self._vipList then
            self:onUpdateVIP()
            return
        elseif tag == IngotLayer.CBT_PROPERTY and 0 ~= #self._propertyList then
            self:onUpdateProperty()
            return
        elseif tag == IngotLayer.CBT_SCORE and 0 ~= #self._scoreList then
            self:onUpdateScore()
            return
        end
...
function IngotLayer:requestPayList(isIap)
    isIap = isIap or 0
            ...
            if type(sjdata) == "table" then
                ...
                if type(sjlist) == "table" then
                    for i = 1, #sjlist do
                        ...
                        item.PayObjectType  = sitem["PayObjectType"] or 0  
...
function IngotLayer:requestPropertyList(nTypeID, tag)
                    ...
                    if valid == true then
                        ...
                        if type(list) == "table" then
                            for i=1,#list do
                                ...
                                if 0 ~= item.id and nil ~= item.id then
                                    if (item.loveliness ~= 0) and (item.bean == 0 and item.gold == 0 and item.ingot == 0) then

                                    else
                                        if tag == IngotLayer.CBT_PROPERTY and item.id == 501 then
                                            if GlobalUserItem.bEnableRoomCard then
                                                item.id = "room_card"
                                                table.insert(tmpList, item)
                                            end
                                        elseif tag == IngotLayer.CBT_SCORE and item.id == 501 and 0 ~= item.resultGold then
                                            if GlobalUserItem.bEnableRoomCard then
                                                item.id = "game_score"
...
function IngotLayer:reloadBeanList()
    self:onClearShowList()
    GlobalUserItem.tabShopCache["shopBeanList"] = {}
    self._beanList = {}
    self:loadPropertyAndVip(IngotLayer.CBT_BEAN)
end

function IngotLayer:updateScoreInfo()
...
--更新游戏币
function IngotLayer:onUpdateScore()
	self:onClearShowList()
	self:onUpdateShowList(self._scoreList,IngotLayer.BT_SCORE)
end

--更新游戏豆
function IngotLayer:onUpdateBeanList()
	self:onClearShowList()
	self:onUpdateShowList(self._beanList,IngotLayer.BT_BEAN)
end

--更新VIP
function IngotLayer:onUpdateVIP()
	self:onClearShowList()
	self:onUpdateShowList(self._vipList,IngotLayer.BT_VIP)
end

--更新道具
function IngotLayer:onUpdateProperty()
	self:onClearShowList()
	self:onUpdateShowList(self._propertyList,IngotLayer.BT_PROPERTY)
end

--更新实物兑换列表
function IngotLayer:onUpdateGoodsList()
...
--清除当前显示
function IngotLayer:onClearShowList()
...
--更新当前显示
function IngotLayer:onUpdateShowList(theList,tag)
	local bGold = (self._select==IngotLayer.CBT_SCORE)
	local bOther= (self._select~=IngotLayer.CBT_SCORE)
	local bBean = (self._select==IngotLayer.CBT_BEAN)

    for i =  IngotLayer.CBT_SCORE,IngotLayer.CBT_ENTITY do 
       -- if self._select ~= i then 
        self:getChildByTag(i):setVisible(false)
       -- end 
    end 
    for i =  IngotLayer.CBT_BEAN_SCORE,IngotLayer.CBT_BEAN_ROOMCARD do    
        self:getChildByTag(i):setSelected(false)  
        if  IngotLayer.BEAN_SCORE == self.m_cbScoreOrRoomCard then
			self:getChildByTag(IngotLayer.CBT_BEAN_SCORE):setSelected(true)
        elseif  IngotLayer.BEAN_ROOMCARD == self.m_cbScoreOrRoomCard then
            self:getChildByTag(IngotLayer.CBT_BEAN_ROOMCARD):setSelected(true)
		end   
--         self:getChildByTag(i):setVisible(true)  
--         self:getChildByTag(i):setEnabled(true) 
    end  
	...
	if #theList<5 then
		scrollHeight = 400
		self._scrollView:setInnerContainerSize(cc.size(1130, 430))
	else
		scrollHeight = 400 * math.ceil(#theList / 4)--math.floor((#theList+math.floor(#theList%3))/3)
	...
	for i=1,#theList do
        ...
        if item.PayObjectType == self.m_cbScoreOrRoomCard or bBean == false then        
		    ...
		    self._showList[nCount] = cc.LayerColor:create(cc.c4b(100, 100, 100, 0), 261, 240)
    	    	:move(160+math.floor((nCount-1)%4)*270-130,scrollHeight-(8+120+math.floor((nCount-1)/4)*400)-150)
    	    	:addTo(self._scrollView)

		    local btn = ccui.Button:create("Ingot/frame_shop_7.png","Ingot/frame_shop_7.png")
            ...
            display.newSprite("Ingot/frame_Ingot_sp_bg_1.png")
		        :move(130,160)
		        :addTo(self._showList[nCount])
            display.newSprite("Ingot/Ingot_bt_duiHuan.png")
		        :move(130,-20)
		        :addTo(self._showList[nCount])
        ...
        local signTxt = ""
   		...
   		if bBean then 
            if bShow then                              
                local cbTypeImg = 0
                local cbTypeText = 0
                if item.PayObjectType == IngotLayer.BEAN_SCORE then --游戏币
                    cbTypeImg = 4
                    cbTypeText = 0                
                elseif item.PayObjectType == IngotLayer.BEAN_BEAN then --游戏豆
                    cbTypeImg = 5
                    cbTypeText = 1
                elseif item.PayObjectType == IngotLayer.BEAN_ROOMCARD then --房卡
                    cbTypeImg = 3
                    cbTypeText = 3
                end 
   			    showSp = display.newSprite("Ingot/icon_shop_" .. cbTypeImg ..".png")
   			    local atlas = cc.LabelAtlas:_create(string.gsub(item.count .. "", "[.]", "/"), "Ingot/num_shop_5.png", 20, 25, string.byte("/"))
   			    atlas:setAnchorPoint(cc.p(1.0,0.5))
   			    self._showList[nCount]:addChild(atlas) 
   			    local name = display.newSprite("Ingot/text_shop_" .. cbTypeText .. ".png")
   			    ...
   			    if nil ~= item.paysend and 0 ~= item.paysend then
   			    	local fsp = cc.Sprite:create("Ingot/shop_firstpay_sp.png")
            ...
            if cc.FileUtils:getInstance():isFileExist("Ingot/title_property_"..item.id..".png") then
                titleSp = display.newSprite("Ingot/title_property_"..item.id..".png")
            end   			

   			--[[local goldLabel = cc.LabelAtlas:_create(string.formatNumberThousands(item.resultGold,true,"/"), "Ingot/num_shop_1.png", 20, 25, string.byte("/")) 
	    		:move(130,220)
	    		:setAnchorPoint(cc.p(0.5,0.5))
	    		:setVisible(bGold)
	    		:addTo(self._showList[i])
	    	display.newSprite("Ingot/sign_shop_2.png")
            ...
            --
            local icon_star = cc.Sprite:create("Ingot/icon_star_sp.png")
        ...
        if bShow or bBean == false then 
		    ...
		    local priceLabel = cc.Label:createWithTTF("需要" .. pricestr .. signTxt, "fonts/round_body.ttf", 24)
            	:setAnchorPoint(cc.p(0.5,0.5))
            	:move(100,-64)
       	    ...
       	    if nil ~= sign then     
                local width = 0  		
                if cc.FileUtils:getInstance():isFileExist("Ingot/sign_shop_"..sign..".png") then
    	           	local spsign = display.newSprite("Ingot/sign_shop_"..sign..".png")
...
function IngotLayer:showPopWait()
	self._scene:showPopWait()
end

function IngotLayer:dismissPopWait()
	self._scene:dismissPopWait()
end

function IngotLayer:queryUserScoreInfo(queryCallback)
