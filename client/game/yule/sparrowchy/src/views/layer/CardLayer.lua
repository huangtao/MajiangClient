...
local posHandCard = {cc.p(430, 660), cc.p(224, 142), cc.p(1271, 0), cc.p(1110, 660)}
local posHandDownCard = {cc.p(295, 665), cc.p(210, 670), cc.p(295, 70), cc.p(1165, 670)}
local posDiscard = {cc.p(780, 495), cc.p(374, 460), cc.p(565, 258), cc.p(970, 292)}
local posBpBgCard = {cc.p(1120, 676), cc.p(248, 668), cc.p(0, 81), cc.p(1111, 154)}
...
function CardLayer:discard(viewId, cardData)
    ...
    -- show arrow
    if self.Arrow_Show_Flag then
        self:removeArrow()
        local arrow=display.newSprite("#sp_arrow.png")
        arrow:move(cc.p(pos.x,pos.y+50)):setTag(500)
        arrow:addTo(self.nodeDiscard[viewId]):setLocalZOrder(500)
    end
...
-- remove Arrow
function CardLayer:removeArrow()
    for i=1,4 do
        if self.nodeDiscard[i] then
            if self.nodeDiscard[i]:getChildByTag(500) then
                self.nodeDiscard[i]:removeChildByTag(500)
                print("viewId="..i)
            end
        end
    end
